...
def loss(policy_dqn: DQN, target_dqn: DQN, states: torch.Tensor, actions: torch.Tensor,
    ...
    bellman_targets = ((~dones).reshape(-1) * (target_dqn(next_states)).max(1).values + rewards.reshape(-1)).cuda()
    q_values = (policy_dqn(states).gather(1, actions).reshape(-1)).cuda()
...
def DQN_process(n_runs, eps, n_episodes, f_update, params, memory, sample_size, training_step=10, lr=1e-3):
    ...
    for run in tqdm(range(n_runs)):
        ...
        policy_net = DQN(params).cuda()
        target_net = DQN(params).cuda()
        update_target(target_net, policy_net)
        ...
        for i_episode in range(n_episodes):
            observation = env.reset()
            state = torch.tensor(observation).float().cuda()
            ...
            while not done:
                ...
                next_state = torch.tensor(observation).reshape(-1).float().cuda()

                memory.push([state, action, next_state, reward, torch.tensor([done]).cuda()])
                ...
                if len(memory.buffer) >= sample_size:
                    ...
                    for _ in range(int(training_step)):
                        ...
                        state_batch, action_batch, next_state_batch, reward_batch, dones = (torch.stack(x).cuda() for x in
                                                                                            zip(*transitions))
...
eps_init_p = 1
eps_min_p = 0.05
eps_decay_p = eps_min_p ** (1 / (num_episodes - 300))

eps_y = []
eps_current = eps_init_p
for e in range(num_episodes):
    eps_current = max(eps_current * eps_decay_p, eps_min_p)
    eps_y.append(eps_current)

plt.plot(np.arange(num_episodes), eps_y, linestyle="-", color="r")
plt.xlabel("Number of episodes", fontsize=18)
plt.ylabel("Epsilon", fontsize=18)
plt.savefig("../report/figures/eps_change.png", bbox_inches="tight")
plt.show()

"""## Plot greedy policy as a function of pole angle and angular velocity
#### Visualising the greedy Q-values for a stationary cart in the middle of the track 2D plot showing policy as a function of pole angle and angular velocity (omega)

#### This plots the policy and Q values according to the network currently stored in the variable "policy_net"
"""

plt.figure(figsize=(15, 10))

policy_net = trained_net  # randomly initialised, replace with your trained DQN
q = True  # whether q values or greedy policy is visualised

angle_range = np.pi / 180 * 12  # pole angle range
...
plt.xlabel("Angle (radius)", fontsize=18)
plt.ylabel("Angular velocity (radius/second)", fontsize=18)
plt.savefig("../report/figures/angle_angV.png", bbox_inches="tight")
plt.show()

"""## Plot the greedy Q-values and policy as a function of cart pole position and velocity
#### Visualising the greedy Q-values for a stationary cart in the middle of the track 2D plot showing policy as a function of cart position and velocity
...
cart_position = 2.4  # cart pole distance to the origin
cart_v = 1  # angular velocity range
